...
class OMSQuery(object):
    ...
    def _attr_exists(self, attr):
        ...
        if self.metadata and attr not in self.metadata:
            self._warn(
                ...
                + "by .set_validation(False) ".format()
            )
    ...
    def _load_meta(self):
        ...
        url = "{base_url}/{resource}/meta".format(
            base_url=self.base_url, resource=resourceBase
        )
        ...
        if response.status_code == 302:
            raise Exception(
                "Received redirect (HTTP 302). Try to switch between http/https protocol"
            )
    ...
    def filter(self, attribute, value, operator="EQ"):
        ...
        if operator not in OMS_FILTER_OPERATORS:
            self._warn(
                "filter() - [{op}] is not supported operator".format(op=operator),
                raise_exc=True,
            )
        ...
        if self._attr_exists(attribute):
            ...
            if searchable:
                self._filter.append(
                    "filter[{k}][{op}]={v}".format(k=attribute, op=operator, v=value)
                )
        return self
    ...
    def get_request(self, url, verify=False):
        if self.oms_auth:
            response = requests.get(
                url,
                verify=verify,
                headers=self.oms_auth.token_headers,
        ...
        if self.err_sec > 0:
            while True:
                try:
                    ...
                    print(
                        "Warning: will retry auth_oidc in "
                        + str(self.err_sec)
                        + "seconds after connection error: "
                        + str(ex)
                    )
    ...
    def auth_oidc_req(self):
        ...
        if self.token_json and self.token_time:
            if current_time - self.token_time < 30:
                print(
                    "Warning: token was requested less than 30 seconds ago. Will not renew this time."
                )
                return
...
class OMSAPI(object):
    ...
    def __init__(
        ...
        self.base_url = "{api_url}/{api_version}".format(
    ...
    def auth_krb(self, cookie_path="ssocookies.txt", sandbox_cmd=False):
        ...
        if not sandbox_cmd:
            ...
            args = [
                ...
                "-c",
                "-l",
                f"{krbenv}auth-get-sso-cookie -u {self.api_url_host} -o {cookie_path} {nocert}",
            ]
