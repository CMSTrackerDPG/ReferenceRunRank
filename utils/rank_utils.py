...
def refrank_pca(
    ...
    wghts = {ftr_name: wght for ftr_name, wght in zip(features.columns, pca.components_[0]**2)}
...
def filter_runs(
    ...
    if json_name is not None:
        goldenruns = map(int, list(json_utils.loadjson(os.path.join(json_path, json_name)).keys()))
...
def filter_runs_runls(
    ...
    run_df, ls_df = filter_by_quality(ls_df, run_df, quality_threshold, json_path, json_name)
...
def lowcorr_highweight(features, corr_threshold, weight_threshold, plot_feats=False, plot_corrs=False):
    """
