...
def test_rank(
    ...
    for i, (index, row) in enumerate(targets_ftrs.iterrows()):
        ...
        if len(candidates) > comp_number:
            rank_df = refrank(
                target_df,
                candidates.loc[: target_df.index[0] - 1].iloc[-comp_number:],
        ...
        try:
            actualRR_rank = rank_df[
                rank_df.index.get_level_values("run") == RR_used
            ].index[0][0]
    ...
    if print_stats:
        ...
        print(
            "Top-k accuracy (k=10): {}".format(
                sum(rank < 10 for rank in RRresults["ranks"]) / len(RRresults["ranks"])
            )
        )
        ...
        print(
            "Mean reciprocal rank: {}".format(
                np.mean([1.0 / (rank + 1) for rank in RRresults["ranks"]])
            )
        )
    ...
    ax.set_title(
        "RRR results for n={}, comparison_num={}".format(n_components, comp_number)
    )
